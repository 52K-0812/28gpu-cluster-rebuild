...
| **대상**      | master-01 ((control-plane-public-ip)), ai-team 네임스페이스, 클러스터 전체 서비스     |
...
- 팀원 2명이 `http://(control-plane-public-ip):8000`으로 JupyterHub 접속 시 **한 명이 로그인하면 다른 한 명의 세션이 끊김**
- 세션 끊김이 서버 기동(Spawn) 단계가 아닌 **로그인 화면(Hub) 단계**에서 발생
- Portainer(`9443`), JupyterHub(`8000`) curl 요청 모두 응답 없음(hang)
- `kubectl logs` 및 `kubectl port-forward` 실행 시 `dial tcp (control-plane-public-ip):10250: i/o timeout` 발생
...
| 4    | `kubectl get svc -n portainer -o yaml` | Portainer `LoadBalancer` 타입, `EXTERNAL-IP: (control-plane-public-ip)` 점거 상태 |
...
MetalLB IP Pool에 `(control-plane-public-ip)`이 등록되어 있었고, Portainer가 이 IP를 `LoadBalancer` 타입으로 점거하고 있었다. `151`은 `master-01` OS 자체가 사용하는 메인 IP이기 때문에 다음 충돌이 발생했다.

```
master-01 OS:   "(control-plane-public-ip)은 내 SSH/터미널 주소야"
...
# type: LoadBalancer, EXTERNAL-IP: (control-plane-public-ip) 확인
```
...
장애의 근본 원인이 IP Pool에 마스터 노드 IP(`151`)가 포함된 것임을 확인했다. 구 클러스터에서 웹서비스 제공용으로 학교 스위치에 이미 허용 등록된 `(control-plane-public-ip)`로 Pool을 교체하면 ARP 충돌 없이 LoadBalancer 타입을 사용할 수 있다고 판단했다.

**🔍 공석 확인:**

```bash
ping -c 5 (control-plane-public-ip)
...
kubectl patch ipaddresspool main-pool -n metallb-system \
  --type='json' \
  -p='[{"op": "replace", "path": "/spec/addresses", "value":["(control-plane-public-ip)/32"]}]'

kubectl get ipaddresspools -n metallb-system
# ADDRESSES: ["(control-plane-public-ip)/32"] 확인
```
...
# EXTERNAL-IP: (control-plane-public-ip) → 즉시 할당 확인

kubectl delete svc ip-test-service
```

**결과:** `EXTERNAL-IP: (control-plane-public-ip)` 정상 할당. 학교 스위치가 해당 IP를 허용하고 있음 검증 완료.
...
| 접근 대상     | 방식         | IP                          | 포트                     |
| ------------- | ------------ | --------------------------- | ------------------------ |
| 학과생 (공용) | LoadBalancer | `(control-plane-public-ip)` | 80 (포트 번호 없이 접속) |
| 관리자 전용   | NodePort     | `(control-plane-public-ip)` | `303xx` 시리즈           |
...
**결과:** 학과생은 `http://(control-plane-public-ip)` 입력만으로 접속 가능. Tailscale 원격 접속은 `http://(vpn-endpoint)30000`.
...
```
ai-team     proxy-public                          LoadBalancer  10.97.60.206   (control-plane-public-ip)  80:30000/TCP
...
| 서비스                 | 접속 주소                          | 비고                                  |
| ---------------------- | ---------------------------------- | ------------------------------------- |
| JupyterHub             | `http://(control-plane-public-ip)` | 포트 번호 없음, 학교 내부망 직접 접속 |
| JupyterHub (Tailscale) | `http://(vpn-endpoint)30000`       | 외부/원격 접속                        |

### 관리자 전용 (151 + 303xx 포트 시리즈)

| 서비스     | 접속 주소                                | 용도                    |
| ---------- | ---------------------------------------- | ----------------------- |
| Grafana    | `http://(control-plane-public-ip):30300` | GPU/Pod 모니터링 시각화 |
| Prometheus | `http://(control-plane-public-ip):30310` | 원본 메트릭 조회        |
| Portainer  | `http://(control-plane-public-ip):30320` | 컨테이너/Pod 관리       |
...
장애 복구 이후 단순히 "동작하는 상태"로 되돌리는 데서 그치지 않고, 서비스 접근 경로를 **목적에 따라 구조적으로 재설계**했다는 점이 핵심이다. 구 클러스터에서 이미 학교 스위치에 허용 등록된 `(control-plane-public-ip)`을 MetalLB Pool로 재활용하고, 더미 서비스로 IP 할당 가능성을 사전 검증한 뒤 실서비스에 적용한 순서가 안전했다.
