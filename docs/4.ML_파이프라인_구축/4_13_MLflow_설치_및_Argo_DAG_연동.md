...
[접속]
    Tailscale: http://(vpn-endpoint)30010
    내부:      http://(control-plane-public-ip):30010
```
...
kubectl patch ipaddresspool main-pool -n metallb-system --type=json -p='[
  {"op": "replace", "path": "/spec/addresses", "value": ["(control-plane-public-ip)/32", "(control-plane-public-ip)/32", "(control-plane-public-ip)/32"]}
]'
      ...
      - name: mlflow
        ...
        args:
          - |
            ...
            mlflow server \
              --backend-store-uri postgresql://mlflow:(password)@mlflow-postgres:5432/mlflow \
...
**원인:** Tailscale VPN은 `100.x.x.x` 대역만 터널링. MetalLB가 ARP로 광고하는 `(control-plane-public-ip)` 대역은 Tailscale subnet router 없이는 외부에서 도달 불가.

**해결:** NodePort + systemd port-forward 방식으로 접속 (JupyterHub, Argo와 동일 패턴 유지)

```
(control-plane-public-ip):30010  → 서버실 내부 접속
(vpn-endpoint)30010   → Tailscale VPN 외부 접속
```
