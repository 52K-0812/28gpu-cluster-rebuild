...
```bash
# 작업 전 상태
v100-gpu-01:    default via (control-plane-public-ip) dev enp2s0f0 (1GbE)
2080ti-gpu-02:  default via (control-plane-public-ip) dev eno1 (1GbE)
2080ti-gpu-03:  default via (control-plane-public-ip) dev eno1 (1GbE)
2080ti-gpu-04:  default via (control-plane-public-ip) dev eno1 (1GbE)
```
...
```bash
scp /tmp/99-k8s-routes-v100.yaml ubuntu@(control-plane-public-ip):/tmp/
ssh ubuntu@(control-plane-public-ip) "sudo cp /tmp/99-k8s-routes-v100.yaml /etc/netplan/99-k8s-routes.yaml && sudo netplan apply"
```
...
```bash
# 2080ti-gpu-02
scp /tmp/99-k8s-routes-2080ti.yaml ubuntu@(control-plane-public-ip):/tmp/
ssh ubuntu@(control-plane-public-ip) "sudo cp /tmp/99-k8s-routes-2080ti.yaml /etc/netplan/99-k8s-routes.yaml && sudo netplan apply"
...
```bash
ssh ubuntu@(control-plane-public-ip) "ip route show | grep 192.168.0.0/16"
ssh ubuntu@(control-plane-public-ip) "ip route show | grep 192.168.0.0/16"
ssh ubuntu@(control-plane-public-ip) "ip route show | grep 192.168.0.0/16"
ssh ubuntu@(control-plane-public-ip) "ip route show | grep 192.168.0.0/16"
```
