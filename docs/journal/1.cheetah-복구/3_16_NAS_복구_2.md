...
| **대상**      | Master Node(`cheetah-master`, MASTER_IP), Storage Node(`nas-cheetah-1`, NAS_1G_IP / NAS_10G_IP) |
...
# 추가 내용:
# /data NAS_10G_IP/24(rw,sync,no_root_squash,no_subtree_check)
...
```bash
sudo mkdir -p /data
echo "NAS_10G_IP:/data /data nfs defaults 0 0" | sudo tee -a /etc/fstab
sudo mount -a                     # 즉시 적용
```
