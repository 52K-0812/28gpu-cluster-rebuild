...
[접속]
    Tailscale: http://TAILSCALE-IP:30600
    내부:      http://MASTER-IP:30600
```
...
| 항목          | 결과                                      |
| ------------- | ----------------------------------------- |
| 배포 노드     | 2080ti-gpu-04 (WORKER-IP-06) |
| GPU           | 2080Ti × 1                                |
| 추론 시간     | **77ms**                                  |
...
**원인:** 브라우저의 `getUserMedia()` API는 **HTTPS 또는 localhost**에서만 동작한다. `http://TAILSCALE-IP:30600` HTTP 접속 시 카메라 권한 차단.
