# 🛡️ [트러블슈팅] 10G NIC 이전 — NAS 네트워크 병목 해소

> **작업 일자:** 2026-03-23
> **대상:** master-02 → NAS (Broadcom BCM57810 PCIe 카드 물리 이전)
> **환경:** Ubuntu 22.04 LTS, NETGEAR ProSafe XS508M 10G 스위치
> **증상:** NAS에 10G NIC 미탑재로 GPU 노드 ↔ NAS 간 실제 통신이 1G로 동작
> **결과:** 전 GPU 노드 ↔ NAS 10G 내부망 구성 완료, ~9Gbps 달성
...
| 항목          | 내용                                                   |
| ------------- | ------------------------------------------------------ |
| **목적**      | GPU 노드 ↔ NAS 간 데이터 전송 병목 해소 (1G → 10G)    |
| **대상**      | master-02 → NAS (Broadcom BCM57810 PCIe 카드 물리 이전) |
| **핵심 전략** | master-02의 잉여 10G NIC를 NAS로 이전, 전 GPU 노드 ~9Gbps 달성 |
...
#### 4-2-1. NAS 10G NIC 인터페이스명 확인 (핵심 주의사항)
...
#### 4-2-3. GPU 노드 10G 설정 (153~156 각각)
...
![자료사진](../images/스크린샷_2026-03-23_190945.png)
