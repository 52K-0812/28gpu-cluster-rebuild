# 🛡️ [트러블슈팅] GPU 노드 네트워크 최적화 — 10GbE 라우팅 설정

> **작업 일자:** 2026-04-02
> **대상:** v100-gpu-01, 2080ti-gpu-02~04 (GPU 워커 노드 전체)
> **환경:** Kubernetes v1.29, Ubuntu 22.04 LTS, netplan
> **증상:** GPU 노드 기본 라우트가 1GbE 인터페이스로 설정 → Calico Pod 통신 병목, DCGM exporter 데이터 미수집
> **결과:** 전 GPU 노드 Pod 네트워크를 10GbE 인터페이스로 우선 라우팅, DCGM 정상화
...
| 항목          | 내용                                                                  |
| ------------- | --------------------------------------------------------------------- |
| **목적**      | GPU 노드 Pod 통신을 10GbE로 우선 라우팅하여 DCGM 안정성 및 학습 성능 확보 |
| **대상**      | v100-gpu-01, 2080ti-gpu-02~04 (GPU 워커 노드 전체)                    |
| **핵심 전략** | netplan metric 조정으로 10GbE 인터페이스 우선 라우팅 강제 설정        |
